...
export async function createCredential(credential: CreateCredentialDto, cipherSecret: string) {
  // Encrypt password
  const password_encryptedData = await encrypt(credential.password, cipherSecret);
  const newCredential: Omit<CredentialFullDto, "id" | "createdAt" | "updatedAt"> = {
    ...credential,
...
export async function getCredentialById(id: string, cipherSecret: string) {
  const credential: CredentialFullDto[] = await credentialsQueries.getCredentialById(parseInt(id));
  if (credential.length === 0) return undefined
  return getCredentialDto(credential[0], cipherSecret);
}

export async function updateCredential(id: string, credential: Partial<CredentialFullDto>, cipherSecret: string) {
  ...
  try {
    const result = await credentialsQueries.updateCredential(parseInt(id), credential);
    revalidateTag(`credentials-${credential.userId}`)
    return result;
  } catch (error) {
    console.error(error);
...
async function getCredentialDto(credential: CredentialFullDto, cipherSecret: string): Promise<CredentialDto> {
  return Promise.all([
...
export type CreateCredentialDto = Omit<CredentialDto, "id" | "createdAt" | "updatedAt">
