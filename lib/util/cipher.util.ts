...
export async function encrypt(textToEncrypt: string, secret: string): Promise<EncryptedData> {
  ...
  const cipher = createCipheriv('aes-256-gcm', keyUint8Array as any, ivUint8Array as any);  // Conversión explícita a any
...
export async function decrypt(secret: string, encryptedData: EncryptedData): Promise<string> {
  let key = keyCache.get(secret);
  if (!key) {
    // Generación de la clave con scrypt
    key = (await promisify(scrypt)(secret, 'salt', 32)) as Buffer;
  ...
  // Usamos 'aes-256-gcm', que es el algoritmo GCM para AES-256
  const decipher = createDecipheriv('aes-256-gcm', keyUint8Array as any, ivUint8Array as any);  // Conversión explícita a any
  decipher.setAuthTag(new Uint8Array(Buffer.from(encryptedData.authTag, 'hex')));

  let decryptedText: string = decipher.update(encryptedData.encryptedText, 'hex', 'utf-8');
